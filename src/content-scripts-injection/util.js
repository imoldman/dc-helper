...
// 发送消息，如成功，返回 (SendMessageError.SUCCESS, null)，
...
async function sendMessageToChannel(guildId, channelId, content, replyingMessageId=null) {
	...
	if (response.status == 200) {
		return[SendMessageError.SUCCESS, null]
	...
	e.addEventListener('keydown', k => {
		if (!k.isComposing && k.code == 'Enter' && !k.shiftKey && isSlowModeOpening()) {
			...
			if (!!content) {
				...
				var replyingMessageId = isReplying() ? getReplyingMessageId() : null;
				let messageItem = new MessageItem(getCurrentGuildId(), getCurrentChannelId(), content, replyingMessageId, (new Date()).getTime());
